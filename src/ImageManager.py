...
HasRootSubdir = BoxInfo.getItem("HasRootSubdir")
HiSilicon = BoxInfo.getItem("HiSilicon")
hasKexec = BoxInfo.getItem("hasKexec")
HasMultibootMTD = BoxInfo.getItem("HasMultibootMTD")
canBackupEMC = BoxInfo.getItem("canBackupEMC")
...
config.imagemanager.folderprefix = ConfigText(default=distro, fixed_size=False)
...
if exists(config.imagemanager.backuplocation.value + "/imagebackups/imagerestore"):
	try:
...
class VISIONImageManager(Screen):
	...
	def doDownload(self):
		try:
			if config.imagemanager.backuplocation.value != "/":
				...
				self.session.openWithCallback(self.doDownloadCallback, MessageBox, message, list=sorted(sorted(choices, key=lambda choice: choice[0]), key=lambda choice: choice[0] == distro, reverse=True), default=0, simple=True)
		except OSError as err:
			print("%s" % err)
	...
	def backupToDelete(self, answer):
		...
		folderprefix = config.imagemanager.folderprefix.value + "-" + str(imagetype)
	...
	def keyRestore1(self):
		...
		if model == "et8500" and exists("/proc/mtd"):
	...
	def keyRestore2(self, retval):
		if retval:
			if canMultiBoot:
				...
				self.MTDKERNEL = canMultiBoot[self.multibootslot]["kernel"].split("/")[2]
				if HasMultibootMTD:
					self.MTDROOTFS = canMultiBoot[self.multibootslot]["device"]
				else:
					self.MTDROOTFS = canMultiBoot[self.multibootslot]["device"].split("/")[2]
			if HiSilicon and getCurrentImage() >= 4 and self.multibootslot < 4:
	...
	def keyRestore3(self, *args, **kwargs):
		...
		if self.sel.endswith(".zip"):
			if config.imagemanager.backuplocation.value != "/" and not exists(self.TEMPDESTROOT):
	...
	def keyRestore4(self, result, retval, extra_args=None):
		if retval == 0:
			...
			if model == "et8500" and self.dualboot:
				...
				if exists("%s/SDAbackup" % MAINDEST) and self.multibootslot != 1:
	...
	def keyRestore6(self, ret):
		...
		if ret == 0:
			...
			if canMultiBoot:
				...
				elif HiSilicon and canMultiBoot[self.multibootslot]["rootsubdir"] is None:	# sf8008 type receiver using SD card in multiboot
				...
				if fileHas("/proc/cmdline", "root=/dev/mmcblk0p1") is True and isfile("%s/rootfs.tar.bz2" % MAINDEST):  # h9 using SD card
					CMD = "/usr/bin/ofgwrite -rmmcblk0p1 '%s'" % MAINDEST
				elif isfile("%s/rootfs.ubi" % MAINDEST) and isfile("%s/rootfs.tar.bz2" % MAINDEST):  # h9 no SD card - build has both roots causes ofgwrite issue
	...
	def ofgwriteResult(self, result, retval, extra_args=None):
		...
		if retval == 0:
			if HiSilicon and HasRootSubdir is False and self.HasSDmmc is False:	# sf8008 receiver 1 eMMC parition, No SD card
			...
			if canMultiBoot:
				...
				if exists(join(tmp_dir, "STARTUP")):
					copyfile(join(tmp_dir, canMultiBoot[self.multibootslot]["startupfile"].replace("boxmode=12'", "boxmode=1'")), join(tmp_dir, "STARTUP"))
				else:
					if exists(config.imagemanager.backuplocation.value + "/imagebackups/imagerestore"):
						try:
							rmtree(config.imagemanager.backuplocation.value + "/imagebackups/imagerestore")
...
class ImageBackup(Screen):
	...
	def __init__(self, session, updatebackup=False):
		...
		self.TMPDIR = self.BackupDirectory + config.imagemanager.folderprefix.value + "-" + str(imagetype) + "-temp"
		self.TMPMOUNTDIR = self.BackupDirectory + config.imagemanager.folderprefix.value + "-" + str(imagetype) + "-mount"
		imageSubBuild = ""
		if imagetype != "develop":
			imageSubBuild = ".%s" % str(imagebuild)
		self.MAINDESTROOT = self.BackupDirectory + config.imagemanager.folderprefix.value + "-" + str(imagetype) + "-" + str(imageSubBuild) + str(visionversion) + "-" + str(visionrevision) + "-" + str(model) + "-" + self.BackupDate
		...
		self.HASKEXEC = hasKexec
		self.MODEL = model
		self.MCBUILD = platform
		self.IMAGEDISTRO = distro
		...
		self.MTDBOOT = "none"
		if canBackupEMC:
			(self.EMMCIMG, self.MTDBOOT) = canBackupEMC
		print("[ImageManager] canBackupEMC:", canBackupEMC)
		self.KERN = "mmc"
		self.rootdir = 0
		if canMultiBoot:
			...
			if HasMultibootMTD:
				self.MTDROOTFS = canMultiBoot[slot]["device"]	# sfx60xx ubi0:ubifs not mtd=
			elif not hasKexec:
				self.MTDROOTFS = canMultiBoot[slot]["device"].split("/")[2]
			if HasRootSubdir and slot != 0:
				self.ROOTFSSUBDIR = canMultiBoot[slot]["rootsubdir"]
		else:
			self.MTDKERNEL = mtdkernel
			self.MTDROOTFS = mtdrootfs
		...
		if "sda" in self.MTDKERNEL:
			self.KERN = "sda"
		print("[ImageManager] hasKexec:", self.HASKEXEC)
	...
	def JobStart(self):
		try:
			if config.imagemanager.backuplocation.value != "/" and not exists(self.BackupDirectory):
...
#						boot = "boot" if slot > 0 and slot < 4 else "dev/%s/%s"  %(self.MTDROOTFS, self.ROOTFSSUBDIR)
						boot = "boot"
						self.command = "dd if=/%s/%s of=%s/vmlinux.bin" % (boot, canMultiBoot[slot]["kernel"].rsplit("/", 1)[1], self.TMPDIR) if slot != 0 else "dd if=/dev/%s of=%s/vmlinux.bin" % (self.MTDKERNEL, self.TMPDIR)
					else:
	...
	def doBackup2(self):
		try:
			...
			if "jffs2" in self.ROOTFSTYPE.split():
				...
				if model in ("h9", "i55plus"):
					with open("/proc/cmdline", "r") as z:
						if HasMMC and "root=/dev/mmcblk0p1" in z.read():
				...
				if canMultiBoot:
					...
					self.commands.append("mount --bind / %s/root" % self.TMPMOUNTDIR)
				if HasRootSubdir and getCurrentImage() == 0:
					self.commands.append("/bin/tar -jcf %s/rootfs.tar.bz2 -C %s/root --exclude ./var/nmbd --exclude ./.resizerootfs --exclude ./.resize-rootfs --exclude ./.resize-linuxrootfs --exclude ./.resize-userdata --exclude ./var/lib/samba/private/msg.sock ." % (self.TMPDIR, self.TMPMOUNTDIR))
				elif HasRootSubdir:
			...
			if exists(self.TMPMOUNTDIR) and exists(self.MAINDESTROOT) and exists(self.TMPDIR) and not ismount(TMPMOUNTDIR + "/root"):
	...
	def doBackup4(self):
		...
		try:
			if exists(self.TMPMOUNTDIR + "/root") and ismount(self.TMPMOUNTDIR + "/root"):
	...
	def doBackup5(self):
		...
		try:
			if self.EMMCIMG == "emmc.img" or self.EMMCIMG == "disk.img" and exists("%s/%s" % (self.TMPDIR, self.EMMCIMG)):
				if exists("%s/%s" % (self.TMPDIR, self.EMMCIMG)):
					move("%s/%s" % (self.TMPDIR, self.EMMCIMG), "%s%s" % (self.MAINDEST, self.EMMCIMG))
				if exists("%s/rootfs.ext4" % self.TMPDIR) and platform != "edision4k":
			...
			with open(self.MAINDEST + "/imageversion", "w") as fileout:
				line = defaultprefix + "-imagebackup-" + str(model) + "_" + self.BackupDate
				fileout.write(line)
			...
			if brand == "vuplus":
				...
				with open(self.MAINDEST + "/noforce", "w") as fileout:
					...
					fileout.write(line)
					fileout.close()
				if isfile("/usr/share/enigma2/receiver/burn.bat"):
					copy("/usr/share/enigma2/receiver/burn.bat", self.MAINDESTROOT + "/burn.bat")
				if HiSilicon and self.KERN == "mmc":
			...
			if exists(self.swapdevice + config.imagemanager.folderprefix.value + "-" + str(imageversion) + "-swapfile_backup"):
			...
			if exists(self.TMPDIR):
				rmtree(self.TMPDIR)
			if (exists(self.MAINDEST + "/" + self.ROOTFSFILE) and exists(self.MAINDEST + "/" + self.KERNELFILE)) or (model in ("h9", "i55plus") and "root=/dev/mmcblk0p1" in z):
				for root, dirs, files in walk(self.MAINDEST):
					for momo in dirs:
						chmod(join(root, momo), 0o644)
					for momo in files:
						chmod(join(root, momo), 0o644)
			...
			if exists(self.MAINDESTROOT) and exists(self.TMPDIR) and not ismount(TMPMOUNTDIR + "/root"):
	...
	def doBackup6(self):
		zipfolder = split(self.MAINDESTROOT)
		try:
			if self.EMMCIMG in ("emmc.img", "disk.img", "usb_update.bin"):
				if self.EMMCIMG in ("emmc.img", "disk.img") and platform != "edision4k" or self.EMMCIMG == "usb_update.bin" and self.ROOTFSSUBDIR.endswith("1"):
	...
	def BackupComplete(self, answer=None):
		...
		import fnmatch
		try:
			if config.imagemanager.number_to_keep.value > 0 and exists(self.BackupDirectory):  # !?!
				images = listdir(self.BackupDirectory)
				...
				if len(emlist) > config.imagemanager.number_to_keep.value:
					...
					for fil in emlist:
						if exists(self.BackupDirectory + fil):
...
class ImageManagerDownload(Screen):
	...
	def getImageDistro(self):
		...
		if not self.imagesList:
			try:
				urljson = join(self.ConfigObj.value, model)
